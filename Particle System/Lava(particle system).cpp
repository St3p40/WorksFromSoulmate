...
void reg(byte id) { //broken
  sparksPos[0][id] = random(0, WIDTH * 10);
  sparksPos[1][id] = genPos[1];
  byte al = random8();
  sparksSpeed[0][id] = (sin(radians(al - 128)) * WIDTH / 2);
  sparksSpeed[1][id] = (cos(radians(al - 128)) * HEIGHT / 8);
  sparksFade[id] = millis();
}

void phisics(byte id) { //broken
  if (SpeedK) {
    if (gravityPos[0] < sparksPos[0][id])
      sparksSpeed[0][id] -= SpeedK / 2;
    else
      sparksSpeed[0][id] += SpeedK / 2;
...
void draw() {
  setGenPos(WIDTH * 5, HEIGHT * 10);
  byte noise = inoise8(millis() / 10);
  setGravityPos(map(abs(128 - noise), 0, 127, WIDTH, WIDTH * 9), 0);
  if (loadingFlag) {
    start();
    loadingFlag = false;
  }
  ...
  for (byte i = 0; i < SPARKS_AM; i++) {
    setRegenRule(i, (sparksPos[0][i] < 0 || sparksPos[1][i] >= (WIDTH * 10) || sparksPos[1][i] < 0) ? 1 : 0);
    render(i, ColorFromPalette(HeatColors_p, 255 - sparksFade[i]));
  }
